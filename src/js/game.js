...
(function () {
  ...
  function updateMovement(dt) {
    ...
    if (isSprinting) {
      const panicDrainMultiplier = 1 + (panic / 100) * 1.5;
      stamina = Math.max(0, stamina - (25 * panicDrainMultiplier * dt)); 
    } else {
    ...
    const spd = (gameState === STATE.QUAKE ? currentSpeed * 0.6 : currentSpeed) * crouchMul * dt;
  ...
  // ── HUD Elements ─────────────────────────────────────────────
  let staminaFill, panicFill, healthFill, timerEl, statusEl, safeEl, outcomeEl;

  function buildHUD() {
    const wrapper = canvas.parentElement;
    
    // We expect the HTML tags to exist now, so we just grab them
    const hint = document.createElement("div"); hint.id = "game-hint";
    hint.innerHTML = `<div class="gh-title">🏢 EARTHQUAKE SCENARIO</div><div class="gh-sub">Click anywhere to enter</div><div class="gh-keys"><span>WASD</span> Move &nbsp;·&nbsp;<span>Shift</span> Sprint &nbsp;·&nbsp;<span>Mouse</span> Look &nbsp;·&nbsp;<span>C</span> Crouch &nbsp;·&nbsp;<span>Esc</span> Exit</div><div class="gh-warn">⚠ An earthquake will strike in 30 seconds. Find cover.</div>`;
    ...
    // Generate Health, Stamina, and Panic wraps since we don't know if the user manually added them in their HTML snippet
